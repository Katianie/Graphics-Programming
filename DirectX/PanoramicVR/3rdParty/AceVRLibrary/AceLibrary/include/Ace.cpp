...
LONGLONG Ace::CalculateFileSize(char* theFilePath)
{
	...
	if (theFilePath != NULL)
	{
		file = CreateFile(theFilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (file == NULL)
		{
...
vector<char*> Ace::RetrieveFileNames(char* fullFilePath)
{
	...
	if (fullFilePath != NULL)
	{
		try
		{
			...
			if ((hFind = FindFirstFile((LPCSTR)filePath, &fd)) != INVALID_HANDLE_VALUE)
			{
				do
				{
